export interface IRisk extends Record<string, unknown> {
  id: string;
  portfolioId: string;
  riskType: "Market Risk" | "Credit Risk" | "Operational Risk" | "Liquidity Risk";
  VaR: number;
...
const RISK_TYPES: IRisk["riskType"][] = [
  "Market Risk",
  "Credit Risk",
  "Operational Risk",
  "Liquidity Risk"
...
export function generateRisk(): IRisk[] {
  ...
  for (let i = 1; i <= numRisks; i++) {
    ...
    const expectedShortfall = Math.round(VaR * (Math.random() * 0.5 + 1.3) * 100) / 100; // 1.3x to 1.8x VaR
    ...
    risks.push({
      ...
      portfolioId: `PORT-${String(Math.floor(Math.random() * 50) + 1).padStart(3, '0')}`,
  ...
  return risks.sort((a, b) => new Date(b.riskDate).getTime() - new Date(a.riskDate).getTime());
}
