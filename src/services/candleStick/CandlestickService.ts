...
export class CandlestickService {
  static async getCandlestickData(symbol: string): Promise<CandleStickEntry[]> {
    ...
    if (!normalizedSymbol.match(/^[a-z]{4}$/)) {
      throw new Error('Invalid symbol format. Symbol must be exactly 4 letters.');
    }

    try {
      const data = await import(`../../data/candleStick/${normalizedSymbol}.json`);
